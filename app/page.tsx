...
import { CONFIG, supportsPdf } from '@/lib/config';
...
// In bring-your-own-key mode the user's key is an Anthropic key, so only Claude
// models are shown. The proxy holds the shared keys, so it shows the full list.
const VISIBLE_MODELS = CONFIG.proxy.enabled
  ? CONFIG.models
  : CONFIG.models.filter((m) => m.provider === 'anthropic');
...
// Labels differ depending on whether a shared-key proxy is configured.
const SECRET_UI = PROXY
  ? {
      chipOn: 'Access set',
      chipOff: 'Add access',
      ...
      help: (
        <>
          RegSpine runs on a shared key held server-side by the RegSpine proxy —
          you don&apos;t need your own API key. Enter the{' '}
          <strong>access password</strong> you were given. It&apos;s stored only in
          this browser.
        </>
      ),
  ...
  : {
      ...
      help: (
        <>
          This app runs entirely in your browser. Your key is saved only in this
          browser (localStorage) and is sent only to Anthropic — never to GitHub or
          any other server. Get one at{' '}
          <a
...
export default function Home() {
  ...
  const addFiles = useCallback((incoming: FileList | File[]) => {
    ...
    for (const file of Array.from(incoming)) {
      ...
      if (!ok) {
        setError(`"${file.name}" is not a PNG/JPEG/WebP/GIF or PDF — skipped.`);
        continue;
      }
      accepted.push({
        file,
  ...
  async function handleRun() {
    ...
    if (!s) {
      ...
      setError(
        PROXY
          ? 'Enter the access password first — it stays in your browser.'
          : 'Add your Anthropic API key first — it stays in your browser.'
      );
      return;
    }
    ...
    // Only Claude reads PDFs; say so before spending a call.
    if (files.some((f) => f.file.type === 'application/pdf') && !supportsPdf(model)) {
      const claude = VISIBLE_MODELS.find(
        (m) => m.provider === 'anthropic' && isAvailable(m.id)
      );
      setError(
        `PDF input is supported only on Claude models.${
          claude ? ` Switch to ${claude.label.split(' — ')[0]}` : ' Switch to a Claude model'
        }, or attach screenshots instead.`
      );
      return;
    }
      ...
      <div className="wrap">
        ...
        {loading ? (
          ...
          <div className="card">
            ...
            >
              <div className="big">
                Drop a screen, communication or PDF here, or click to browse
              </div>
              <div className="small">
                PNG · JPEG · WebP · GIF · PDF — up to {MAX_TOTAL_MB} MB total
              </div>
              <input
                ref={inputRef}
                type="file"
        ...
        {!result && !loading && (
          <p className="footer-note">
            ...
            {PROXY ? ' via the RegSpine shared-key proxy' : ''}. Every audit is a
            live model call on the artifact you upload — nothing is cached or
            pre-computed. Informs design and compliance decisions — not legal
            sign-off.
          </p>
        )}
      </div>
...
function humaniseError(e: unknown): string {
  ...
  if (/401|authentication|invalid x-api-key|invalid_api_key/i.test(msg))
    return PROXY
      ? 'The proxy rejected the request (401). Check the access password in settings.'
  ...
  if (/not set on the proxy/i.test(msg))
    ...
    return `${msg} Pick a model from a provider that has a key loaded, or run \`npm run sync-secrets\`.`;
  ...
  if (/Failed to fetch|NetworkError|CORS/i.test(msg))
    return PROXY
      ? 'Network/CORS error reaching the proxy. Confirm the proxy URL and that this site origin is allowed.'
      : 'Network/CORS error reaching Anthropic. Check your connection and that the key is valid.';
  return msg;
}
